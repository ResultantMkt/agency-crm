...
interface Metrics {
  ...
  conversionRates: {
    leadToMql: number
    mqlToScreening: number
    screeningToClosingMeeting: number
    closingMeetingToClose: number
  }
}
...
function MonthGrid({
  year, month, pendingFrom, pendingTo, hoverDate, selectingSecond, maxDate, onDayClick, onDayHover,
}: {
  year: number; month: number; pendingFrom: string | null; pendingTo: string | null
  hoverDate: string | null; selectingSecond: boolean; maxDate: string
  ...
  const [rangeStart, rangeEnd] =
    pendingFrom && effectiveTo
      ? pendingFrom <= effectiveTo ? [pendingFrom, effectiveTo] : [effectiveTo, pendingFrom]
      : [null, null]
...
function DateRangePicker({ dateFrom, dateTo, onApply }: {
  ...
  function openPicker() {
    ...
    setSelectingSecond(false); setHoverDate(null)
    setViewLeft(initViewDate()); setOpen(true)
  }
  function handleDayClick(ds: string) {
    if (!selectingSecond) { setPendingFrom(ds); setPendingTo(null); setSelectingSecond(true) }
    else {
      const [f, t] = ds < pendingFrom! ? [ds, pendingFrom!] : [pendingFrom!, ds]
      setPendingFrom(f); setPendingTo(t); setSelectingSecond(false)
    }
  }
  function handleApply() {
    if (pendingFrom && pendingTo) { onApply(pendingFrom, pendingTo); setOpen(false) }
  }
  function prevMonth() {
    setViewLeft((v) => v.month === 1 ? { year: v.year - 1, month: 12 } : { year: v.year, month: v.month - 1 })
  }
  function nextMonth() {
    setViewLeft((v) => v.month === 12 ? { year: v.year + 1, month: 1 } : { year: v.year, month: v.month + 1 })
  }
        ...
        <div className="absolute right-0 top-full z-50 mt-2 flex overflow-hidden rounded-xl border border-gray-700 bg-gray-900 shadow-2xl">
          ...
          <div className="flex flex-col p-4">
            ...
            <div className="flex gap-6" onMouseLeave={() => setHoverDate(null)}>
              <MonthGrid year={viewLeft.year} month={viewLeft.month} pendingFrom={pendingFrom} pendingTo={pendingTo}
                hoverDate={hoverDate} selectingSecond={selectingSecond} maxDate={todayStr}
                onDayClick={handleDayClick} onDayHover={setHoverDate} />
              <MonthGrid year={rightYear} month={rightMonth} pendingFrom={pendingFrom} pendingTo={pendingTo}
                hoverDate={hoverDate} selectingSecond={selectingSecond} maxDate={todayStr}
                onDayClick={handleDayClick} onDayHover={setHoverDate} />
            </div>
...
function InvestmentCard({ periodKey, value, onSave }: {
  periodKey: string; value: number; onSave: (v: number) => Promise<void>
}) {
...
export function DashboardClient() {
  ...
  useEffect(() => { fetchMetrics(dateFrom, dateTo) }, [dateFrom, dateTo, fetchMetrics])
        ...
        <>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
              ...
              <ConversionCard from="MQL" to="Triagem Ag." rate={conv?.mqlToScreening ?? 0} />
              <ConversionCard from="Triagem Real." to="Reun. Fechamento" rate={conv?.screeningToClosingMeeting ?? 0} />
              <ConversionCard from="Reun. Fechamento" to="Fechamento" rate={conv?.closingMeetingToClose ?? 0} />
            </div>
          </section>
        </>
      )}
