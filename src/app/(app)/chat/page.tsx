...
function ContactAvatar({ name, photoUrl }: { name: string; photoUrl?: string | null }) {
  ...
  return (
    <div className={`h-10 w-10 rounded-full flex items-center justify-center shrink-0 text-gray-900 text-sm font-semibold ${avatarColor(name)}`}>
      {getInitials(name) || "?"}
    </div>
  )
}
...
function ConvItem({ conv, isSelected, photoUrl, onSelect, onAction }: ConvItemProps) {
  ...
  return (
    <div className={`relative group flex items-center gap-3 px-3 py-2.5 cursor-pointer transition-colors hover:bg-gray-200/30 ${isSelected ? "bg-gray-100 border-l-2 border-purple-500" : "border-l-2 border-transparent"}`}>
      ...
      <div className="flex-1 min-w-0" onClick={onSelect}>
        <div className="flex items-center justify-between gap-1">
          <div className="flex items-center gap-1.5 min-w-0">
            {conv.pinned && <Pin className="h-3 w-3 text-purple-600 shrink-0" />}
      ...
      <div ref={menuRef} className="shrink-0 relative">
        <button
          ...
          className="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-gray-200/50 text-gray-500 hover:text-gray-800 transition-all"
        >
          <MoreVertical className="h-4 w-4" />
        </button>

        {menuOpen && (
          <div className="absolute right-0 top-7 z-50 w-44 rounded-lg border border-gray-200 bg-white shadow-2xl py-1">
            <button
              type="button"
            ...
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-red-400 hover:bg-gray-100 hover:text-red-300"
            >
...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] overflow-hidden rounded-lg border border-gray-200/50">
      {/* ── Sidebar ── */}
      <div className="w-[300px] shrink-0 flex flex-col border-r border-gray-200/50 bg-gray-100/30">

        {/* Header */}
        <div className="shrink-0 px-4 pt-4 pb-3 border-b border-gray-200/50 space-y-3">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-xs bg-gray-100 border border-gray-200 text-gray-900 rounded-lg focus:outline-none focus:ring-1 focus:ring-purple-500 placeholder:text-gray-400"
            />
          </div>
        </div>
              ...
              {archived.length > 0 && (
                <div>
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-4 py-2.5 text-xs text-gray-500 hover:text-gray-600 hover:bg-gray-200/20 transition-colors"
                  >
                    ...
                    <span className="ml-auto bg-gray-200 text-gray-500 text-[10px] px-1.5 py-0.5 rounded-full">{archived.length}</span>
                  </button>
                  {archivedOpen && (
